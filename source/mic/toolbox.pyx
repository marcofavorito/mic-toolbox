...
def train(object T, DataStruct data_struct, int mem, int lags, double d,
    ...
    t_start      = time.time()    # Start overall timer
    ...
    for head_loc in range(T.lags,N):
        ...
        if N_count > N_crit-1:       # Check against percentile value
            time_percent[p_count] = time.time() - t_start;
    ...
    print(divider)
    time_tot = time.time() - t_start   # Save total time
...
def particle_run(list tree_list, list lb, list ub, list r_vec,
    ...
    tot_start = time.time()          # Start overall timer
    ...
    for t in range(Nt):
        ...
        time_t = time.time() - t_start   # Save iteration time
        print(' Iter: {:4d}\tTime required : {:10.4f}'.format(t,time_t))


    time_tot = time.time() - tot_start   # Save total time
